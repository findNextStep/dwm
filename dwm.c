...
static Monitor *dirtomon_with_end(int dir);
...
static void focus_client(const Arg * arg);
...
focusmon(const Arg *arg)
{
	...
	if (!mons->next)
		return;
	if ((m = dirtomon_with_end(arg->i)) == selmon)
		return;
    if (m == NULL)
        return;
	unfocus(selmon->sel, 0);
